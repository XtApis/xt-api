...
const translations = {
  'zh-Hans': {
    ...
    'Enter at least 2 characters to start searching': '输入至少 2 个字符开始搜索',
    'Search Tips:': '搜索提示：',
    'Search API interface names (e.g.: futures trading, spot trading, copy trading)': '搜索 API 接口名称（如：期货交易、现货交易、跟单交易）',
    'Search function keywords (e.g.: order, cancel, query, WebSocket)': '搜索功能关键词（如：订单、取消、查询、WebSocket）',
    'Search parameter names (e.g.: price, quantity, order, position)': '搜索参数名称（如：价格、数量、订单、持仓）',
    'Search document types (e.g.: basic info, error codes, signature algorithm)': '搜索文档类型（如：基础信息、错误码、签名算法）',
...
function translateHtmlContent(content, locale) {
  ...
  Object.entries(localeTranslations).forEach(([en, translated]) => {
    ...
    const onclickRegex = new RegExp(`onclick="showCategory\\('${en.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}'\\)"`, 'g');
    translatedContent = translatedContent.replace(onclickRegex, `onclick="showCategory('${en}')"`);
  });
...
function translateDocusaurusConfig(locale) {
  ...
  // 保存翻译后的配置
  const translatedConfigPath = path.join(__dirname, `../docusaurus.config.${locale}.ts`);
