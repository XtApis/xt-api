/**
 * 索引 Docusaurus 文档到 Algolia
 * 这个脚本会扫描 docs 文件夹中的所有 Markdown 文件，
 * 提取内容并上传到 Algolia 索引中
 */
...
// Algolia 配置
const client = algoliasearch('GHKOVREISY', '243cbab4da40e6e70ad34e29aad67325');
...
function processMarkdownFile(filePath) {
  ...
  try {
    ...
    finalUrl = `/docs/${relativePath.replace(/\.(md|mdx)$/, '').replace(/\\/g, '/')}`;
  }
...
function validateAndSplitRecords(records) {
  ...
  records.forEach(record => {
    ...
    if (recordSize <= maxSize) {
      ...
      // 分割过大的记录
      console.log(`⚠️  记录 "${record.title}" 太大 (${recordSize} bytes)，正在分割...`);
...
async function indexDocuments() {
  try {
    ...
    // 处理所有文件
    const rawRecords = markdownFiles.map(processMarkdownFile).filter(record => {
      // 过滤掉有问题的记录
