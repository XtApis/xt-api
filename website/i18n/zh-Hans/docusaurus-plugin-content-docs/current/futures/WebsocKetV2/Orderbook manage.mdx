...
### How to Manage a Local 订单 Book Correctly
...
Due to depth snapshots having a **limit on the 数字 of price levels**, a price level outside of the initial snapshot that doesn't have a quantity change will not have an update in the Diff Depth Stream.

Consequently, those price levels will not be visible in the local order book even when applying all updates from the Diff Depth Stream correctly, which may cause **slight differences** compared to the 实时 order book.
