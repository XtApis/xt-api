---
id: GetWsToken
title: 获取令牌
sidebar_label: 获取令牌
sidebar_position: 4
---

**类型:** POST  
**描述:** `/v4/ws-token`

### 参数

| 名称 | 类型 | 必填 | 默认值 | 描述 | 范围 |
| ---- | ---- | --------- | ------- | ---------------------- | ------ |
| –    | –    | false     | N/A     | 无需参数 | –      |

### 注意事项

- 限流规则: **1 request / 10s / apikey**
- `accessToken` 有效期为 **2天**。
- 再次调用此接口将**重置有效期**。
- `accessToken` 等同于 **listenKey**，用于建立WebSocket连接。

### 响应示例

```json title="Response"
{
  "rc": 0,
  "mc": "SUCCESS",
  "ma": [],
  "result": {
    "accessToken": "eyJhbqGciOiJSUzI1NiJ9..h3zJlJBQrK2x1HvUxsKivnn6PlSrSDXXXJ7WqHAYSrN2CG5XPTKc4zKnTVoYFbg6fTS0u1fT8wH7wXqcLWXX71vm0YuP8PCvdPAkUIq4-HyzltbPr5uDYd0UByx0FPQtq1exvsQGe7evXQuDXx3SEJXxEqUbq_DNlXPTq_JyScI",
    "refreshToken": "eyJhbGciOiqJSUzI1NiJ9..Fs3YVm5YrEOzzYOSQYETSmt9iwxUHBovh2u73liv1hLUec683WGfktA_s28gMk4NCpZKFeQWFii623Fvdf注意XR0v1yZ2519uNvNndtuZICDdv3BQ4wzW1wIHZa1skxFfqvsDnGdXpjqu9UFSbtHwxprxeYfnxChNk4ssei430"
  }
}
```
